...
def test_snowflake_connector(mock_connect):
    config = WarehouseConfig(type="snowflake", account="acc", username="usr", password="pw")
    connector = SnowflakeConnector(config)
    
    # Mock cursors
    mock_cursor = mock_connect.return_value.cursor.return_value
    mock_cursor.fetchall.return_value = [
        ("PUBLIC", "USERS", 100, True)
    ]
...
def test_bigquery_connector(mock_client_cls):
    ...
    mock_job.result.return_value = [
        MockRow(schemaname="public", tablename="users", row_count=100, has_timestamps=True)
    ]
...
def test_dbt_connector_success(tmp_path):
    ...
    rr = {
        "results": [
            {"unique_id": "model.test.users", "status": "success"}
        ]
    }
...
def test_airflow_connector(mock_httpx_client):
    ...
    mock_response_1.json.return_value = {"dags": [{"dag_id": "dag1", "is_active": True, "owners": ["me"]}], "total_entries": 1}
