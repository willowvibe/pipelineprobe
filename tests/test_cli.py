...
def test_cli_init():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert os.path.exists("pipelineprobe.yml"), (
                "pipelineprobe.yml was not created"
            )
        finally:
            os.chdir(old_cwd)
...
""")
            ...
            with (
                patch("pipelineprobe.cli.AirflowConnector") as mock_airflow_cls,
                ...
                result = runner.invoke(
                    app, ["audit", "--format", "json", "--fail-on-critical", "0"]
                )
...
def test_cli_audit_invalid_format():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            assert result.exit_code != 0
            assert (
                "Must be one of: both, html, json" in result.stdout
