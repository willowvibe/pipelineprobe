...
from pipelineprobe.rules.airflow_rules import (
    check_missing_retries,
    check_missing_slas,
    check_stale_dags,
    check_high_failure_rate,
)
from pipelineprobe.rules.postgres_rules import (
    check_large_tables,
...
def test_check_high_failure_rate():
    dags = [
        ...
        Dag(
            ...
            recent_runs=[
                DagRun(
                    state="failed", start_time=datetime.now(), end_time=datetime.now()
                ),
                DagRun(
                    state="failed", start_time=datetime.now(), end_time=datetime.now()
                ),
                ...
                DagRun(
                    state="success", start_time=datetime.now(), end_time=datetime.now()
                ),
                DagRun(
                    state="success", start_time=datetime.now(), end_time=datetime.now()
                ),
...
def test_check_large_tables(mock_postgres_tables):
    context = {"warehouse_tables": mock_postgres_tables, "warehouse_type": "postgres"}
...
def test_check_missing_timestamps():
    ...
    issues = check_missing_timestamps(
        {"warehouse_tables": tables, "warehouse_type": "postgres"}
    )
    assert len(issues) == 1
